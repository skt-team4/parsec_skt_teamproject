...
const TMAP_API_KEY = 'F1YRiyhnpe2JhzqhlotAu8LF56DUKeQD9cA4Uxxb'; // T-Map API 키
...
class TmapService {
  ...
  /**
   * 주소 검색 (POI 통합 검색)
   * @param {string} searchText - 검색할 주소나 장소명
  ...
  async searchAddress(searchText) {
    try {
      ...
      const results = data.searchPoiInfo.pois.poi.map(poi => ({
        ...
        fullAddress: poi.roadName || poi.fullAddressRoad || poi.upperAddrName + ' ' + poi.middleAddrName + ' ' + poi.lowerAddrName,
        lat: parseFloat(poi.noorLat),
        lng: parseFloat(poi.noorLon),
        telNo: poi.telNo || '',
        category: poi.upperBizName || '',
      }));
      ...
      console.error('TMAP POI Search API Error:', error);
      return {
        success: false,
        error: error.message,
        results: [],
  ...
  async reverseGeocode(lat, lng) {
    try {
      const url = `${this.baseURL}/geo/reversegeocoding`;
      const params = new URLSearchParams({
        version: '1',
        lat: lat.toString(),
        lon: lng.toString(),
      ...
      return {
        ...
        city: data.addressInfo.city_do,
        gu: data.addressInfo.gu_gun,
        dong: data.addressInfo.legalDong,
        roadName: data.addressInfo.roadName,
        buildingNo: data.addressInfo.buildingNo,
      };
    } catch (error) {
      console.error('TMAP Reverse Geocoding API Error:', error);
      return {
        success: false,
        error: error.message,
      };
